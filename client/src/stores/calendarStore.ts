...
interface CalendarState {
  ...
  addCategory: (category: Omit<Category, '_id' | 'createdAt' | 'createdBy'>) => Promise<void>;
  updateCategory: (id: string, updates: Partial<Omit<Category, '_id' | 'createdAt' | 'createdBy'>>) => Promise<void>;
      ...
      loadSpecialDays: async () => {
        try {
          ...
          const updatedSpecialDays = specialDays.map((day: SpecialDay) => {
            const category = get().categories.find(c => c._id === day.category?._id);
            if (!category) {
          ...
          set({ error: "Error loading special days", isLoading: false, specialDays: [] });
        }
      },
